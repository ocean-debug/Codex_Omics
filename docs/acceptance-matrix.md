...
| nf-core | Registry, schema fetch, params fallback validation, command generation, `rnaseq/sarek/atacseq` samplesheets, output inventory, project-local Java/Nextflow activation, project-local Singularity cache/config, runtime blockers and Nextflow failure classification | Command generation, structured environment inspection, and output inventory | `nf-core/rnaseq` test profile and real bulk RNA subset with MultiQC inventory must complete when preflight passes; ATAC may require pre-cached `nf-core/atacseq` or a compatible Nextflow/pipeline revision | Every nf-core pipeline end-to-end |
...
`scripts/acceptance/run_all.sh` writes `summary.json` after attempting scVI,
bulk RNA, and ATAC checks. scVI and bulk RNA are required to complete. ATAC is
accepted only if it completes or the manifest records a known classified
pipeline pull, pipeline config parse, or container pull failure with saved
commands and logs.

...
| nextflow-development | env check, FASTQ detection, samplesheet, command build, optional pull-timeout config, approved execution wrapper | RNA-seq reached real nf-core tasks on gpu03; container pull timeout classified; ATAC plan completed | guaranteed HPC execution or remote container registry performance |

## Remote nf-core note

The gpu03 RNA-seq acceptance run confirmed the plugin command path and nf-core execution, but the remote server's container downloads from `depot.galaxyproject.org` were slow enough to exceed Nextflow's default `singularity.pullTimeout` of `20m` for several uncached images. Use `build_nextflow_command.py --pull-timeout "4 h" --overwrite-reports` or pre-cache required Singularity/Apptainer images before `-resume` on slow HPC networks.
